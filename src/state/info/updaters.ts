...
export const PoolUpdater: React.FC = () => {
  ...
  const { loading: poolAddrLoading, error, addresses } = useTopPoolAddresses()

  // add top pools on first load
  useEffect(() => {
    if (addresses && !error && !poolAddrLoading) {
      addPoolKeys(addresses)
    }
  }, [addPoolKeys, addresses, error, poolAddrLoading])
  ...
  const { error: poolDataError, loading, data: poolDatas } = usePoolDatas(unfetchedPoolAddresses)
