...
import { ProtocolData, TokenData, ChartEntry, PriceChartEntry } from './types'
// Protocol hooks
...
export const usePoolsForToken = (address: string): string[] | undefined => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const { error: fetchError, addresses } = await fetchPoolsForToken(address, dataClient)
  ...
  }, [address, dispatch, error, poolsForToken, dataClient])
