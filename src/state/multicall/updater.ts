...
import { useActiveWeb3React, useMulticallContract } from '../../hooks/useContracts'
import useDebounce from '../../hooks/useDebounce'
import chunkArray from '../../utils/chunkArray'
import { CancelledError, retry, ReturnableError } from '../../utils/retry'
...
async function fetchChunk(
  ...
  if (resultsBlockNumber.toNumber() < minBlockNumber) {
    ...
    throw new ReturnableError('Fetched for old block number')
  }
  ...
  // try {
  //   const data = await multi.methods.aggregate([chunk[0]].map((obj) => [obj.address, obj.callData])).call();
  //   resultsBlockNumber = Number(data.blockNumber);
  //   // eslint-disable-next-line prefer-destructuring
  //   returnData = data.returnData;
  //   // [resultsBlockNumber, returnData] = await multicallContract.aggregate(
  //   //   [chunk[0]].map((obj) => [obj.address, obj.callData])
  //   // )
  // } catch (error) {
  //   console.info('Failed to fetch chunk inside retry', error)
  //   throw error
